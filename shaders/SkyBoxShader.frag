...
uniform samplerCube cubemapDepthTexture;
uniform samplerCube cubemapSkyTexture;
uniform samplerCube cubemapNormalTexture;
...
vec4 getSky()
{
    vec2 v = vec2(dot(cameraUp,TexCoord0),dot(cameraSide,TexCoord0) );
...
void main()
{
    vec3 vertex = TexCoord0;
    float z_comp = texture(cubemapDepthTexture, TexCoord0).x;
    vertex -= (z_comp) * cameraDir / 2.0f;
    
    vec3 total_light = vec3(0.0f);
    vec3 eye_dir = normalize(cameraPosition - vertex);
    total_light = texture(cubemapTexture, TexCoord0).xyz * 0.6;
    
    vec3 light_dir = normalize(lightPos - vertex);
    vec3 normal = normalize(texture(cubemapNormalTexture, TexCoord0).xyz);
    vec3 kd = texture(cubemapTexture, TexCoord0).xyz * 0.8;
    vec3 ks = texture(cubemapTexture, TexCoord0).xyz * 0.8;
    ...
    outColor = scene_light * (1.0f - texture(cubemapSkyTexture, TexCoord0)) + sky_light*texture(cubemapSkyTexture, TexCoord0);
}
