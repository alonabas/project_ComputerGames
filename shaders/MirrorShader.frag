...
const vec3 lightWood  = vec3(0.4,0.2,0.1);
const vec3 darkWood = vec3(0.3,0.1,0);

uniform vec3 light_position[3];
uniform vec3 light_color[3];

uniform samplerCube textureData;
uniform sampler2D textureGhost;
uniform int type;
uniform float time;

const float turbulence = 2.71f;
...
void main()
{
    ...
    if(type == 0){
        vec3 ref = normalize(reflect(normalize(vertex),normalize(normal_vertex)));
        vec3 refraction = refract(normalize(vertex), normalize(normal_vertex), 0.9f);
        
        float F = ((1.0-0.02f) * (1.0-0.02f)) / ((1.0+0.02f) *(1.0 +0.02f));
        float Ratio = F + (1.0 -F) * pow((1.0 - dot(-vertex,normal_vertex)), 0.8f);
        my_kd = mix(texture(textureData,ref).xyz,texture(textureData,refraction).xyz,Ratio);
        my_ks = vec3(0.9, 0.9, 0.9);
        my_shinisess = 2000.0f;
        for(int i = 0; i<3; i++){
            total_light += getLight(i);
        }
        vec4 ghost = texture(textureGhost,vec2(tex_coord.x,1.0f-tex_coord.y));
        outColor = mix(ghost, vec4(total_light,1.0f),0.2f + time/62.5f);
        outColor.a = 1.0f;
    }
    else{
