...
public:
	...
	;
	Bpixel(uchar pr, uchar pg, uchar pb) {
		r = pr;
		g = pg;
		b = pb;
	}
	...
	;
	Bpixel(const Bpixel& other) {
		r = other.r;
		g = other.g;
		b = other.b;
	}
	Bpixel& operator=(const Bpixel& other) {
		r = other.r;
		g = other.g;
		b = other.b;
		return *this;
	...
	Bpixel(Bpixel& p1, double w1, Bpixel& p2, double w2, Bpixel& p3, double w3,
			Bpixel& p4, double w4) {
...
public:
	...
	int read(const char * image_name) {
		return readImage(image_name);
	}
	int load(const char * image_name) {
		return readImage(image_name);
	}

	int writeImage(const char * image_name);
	int write(const char * image_name) {
		return writeImage(image_name);
	}
	int save(const char * image_name) {
		return writeImage(image_name);
	}
	...
	void blt(BImage& source, int x_src, int y_src, int x_dst, int y_dst,
...
inline uchar BImage::operator()(double x, double y, int col) {
	...
	return (uchar) (operator()(ix, iy, col) * (1 - dx) * (1 - dy)
			+ operator()(ix + 1, iy, col) * dx * (1 - dy)
			+ operator()(ix, iy + 1, col) * (1 - dx) * dy
			+ operator()(ix + 1, iy + 1, col) * dx * dy + 0.5);
}
